...
`InstantReload` is a client-only Fabric mod for **Minecraft Java 1.20.1** that replaces all-at-once atlas rebuild behavior with a **selective atlas reload pipeline**.
...
- For unchanged atlases:
  ...
  - skips `SpriteAtlasTexture.upload(...)` GPU update path
...
- Client-side only.
- Targets the 1.20.1 Yarn surface used in this folder.
