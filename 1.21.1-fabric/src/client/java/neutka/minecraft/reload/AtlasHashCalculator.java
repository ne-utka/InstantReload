...
public final class AtlasHashCalculator {
	...
	public static AtlasScanResult scan(
		...
		Collection<ResourceMetadataReader<?>> additionalMetadata
	) {
		...
		List<java.util.function.Function<SpriteOpener, SpriteContents>> sources = loader.loadSources(resourceManager);
		...
		for (int i = 0; i < sources.size(); i++) {
			java.util.function.Function<SpriteOpener, SpriteContents> source = sources.get(i);
			accumulator.updateInt(i);
			accumulator.updateUtf8(source.getClass().getName());

			SpriteContents contents = source.apply(hashingOpener);
			if (contents == null) {
